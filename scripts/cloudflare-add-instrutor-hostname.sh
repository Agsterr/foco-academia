...
HTTP=$(curl -sS -o /tmp/cf_instrutor_tunnel.json -w '%{http_code}' \
...
if [[ "$HTTP" != "200" ]]; then
  ...
  cat /tmp/cf_instrutor_tunnel.json
  exit 1
fi
...
bash "${SCRIPT_DIR}/cloudflare-dns-instrutor.sh" || true

echo "Aguarde ~30s e teste:"
echo "  curl -sI https://${HOSTNAME}/login"
echo "  curl -s https://${HOSTNAME}/manifest.json"
