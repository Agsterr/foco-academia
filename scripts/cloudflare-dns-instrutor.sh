...
Opcao B — exporte o zone_id e rode de novo:
  ...
  bash scripts/cloudflare-dns-instrutor.sh
EOF
  exit 1
fi
...
if [[ -n "$existing" ]]; then
  echo "OK: CNAME ${DNS_NAME} ja existe."
  exit 0
fi

code=$(curl -sS -o /tmp/cf_dns_instrutor.json -w '%{http_code}' \
  -X POST "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records" \
  -H "Authorization: Bearer ${API_TOKEN}" \
  -H "Content-Type: application/json" \
  --data "{\"type\":\"CNAME\",\"name\":\"${DNS_NAME}\",\"content\":\"${TUNNEL_CNAME}\",\"proxied\":true}")

if [[ "$code" == "200" ]]; then
  echo "OK: CNAME ${DNS_NAME} -> ${TUNNEL_CNAME} criado."
else
  echo "ERRO: falha CNAME ${DNS_NAME} (HTTP ${code})"
  cat /tmp/cf_dns_instrutor.json
  exit 1
fi
